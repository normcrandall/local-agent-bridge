...
const reviewEventMode = prompt === "ambiguous review publication"
  ? "ambiguous"
  : process.env.FAKE_CODEX_REVIEW_TOOL_EVENT;
const events = [
  ...
  ...(reviewEventMode ? [
    { type: "item.started", item: { id: "review-1", type: "mcp_tool_call", server: "github_review", tool: "submit_pr_review", status: "in_progress" } },
    { type: "item.completed", item: { id: "review-1", type: "mcp_tool_call", server: "github_review", tool: "submit_pr_review", ...(reviewEventMode === "ambiguous" ? {} : { status: "completed" }) } },
  ] : []),
