...
// provider that cannot enforce exact command grants (Codex) is denied with a typed error
// BEFORE any provider process is spawned. Antigravity's owner-selected policy is tested
// separately: command-running reviews automatically use its unrestricted CLI profile.
...
try {
  ...
  // reviewer is meant to run. Codex has no enforceable grant for it.
