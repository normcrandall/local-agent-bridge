...
check("Provider concurrency policy", () => {
  ...
  return info.isFile()
    && (info.mode & 0o077) === 0
    && config.version === 1
    && ["claude", "codex", "antigravity", "ollama", "docker"].every((provider) => {
...
check("Docker Model Runner local reviewer", () => {
  ...
  const status = spawnSync(docker, ["model", "status"], { encoding: "utf8", timeout: 10_000 });
  if (status.status !== 0) return false;
  const models = spawnSync(docker, ["model", "list"], { encoding: "utf8", timeout: 10_000 });
  return models.status === 0 && /\S+/.test(models.stdout || "");
}, "enable Docker Model Runner and pull at least one local model, or set DOCKER_BIN");
...
}, "register codex, claude_code, ollama, docker, and collaboration in ~/.gemini/config/mcp_config.json");
...
check("Global collaboration launcher executable", () => {
  for (const name of ["claude", "codex", "antigravity", "ollama", "docker", "collaboration", "playwright"]) {
