...
import {
  ...
  DEFAULT_DOCKER_MODEL_RUNNER_CONFIG,
...
try {
  ...
  const contract = await probeDockerModelRunnerContract({
    model: "ai/qwen2.5-coder",
    fetchImpl: async (url, request = {}) => {
      contractRequests.push({ url, request });
  ...
  await assert.rejects(
    runLocalReview({
      ...
      fetchImpl: async () => { throw new Error("must not reach inference"); },
    }),
    /provider-specific local model configuration/,
  );
  ...
  const responses = [
    {
      ...
      message: {
        role: "assistant",
        content: "",
        tool_calls: [{ id: "call-1", function: { name: "read_file", arguments: JSON.stringify({ path: "app.mjs" }) } }],
      },
    },
    {
      model: "docker.io/ai/qwen2.5-coder:latest",
  ...
  if (dockerStatus && dockerStatus.status === 0) {
    ...
    await probeDockerModelRunnerContract({
      model: liveConfiguration.model,
  ...
  try {
    ...
    const reviewUnavailable = await client.callTool({
      name: "ask_docker",
      arguments: { prompt: "Review this repository", cwd: ".", mode: "review" },
    });
    assert.equal(reviewUnavailable.isError, true);
    assert.equal(reviewUnavailable.structuredContent.reason, "configuration_error");
    assert.match(reviewUnavailable.content[0].text, /configuration_error/);
    assert.doesNotMatch(JSON.stringify(reviewUnavailable), new RegExp(repository));
  } finally {
    await client.close();
  }
