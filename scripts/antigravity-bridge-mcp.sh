...
if [[ -n "${NODE_BIN:-}" && -x "$NODE_BIN" ]]; then
  exec "$NODE_BIN" "$ROOT/src/antigravity-bridge.mjs"
fi

if command -v node >/dev/null 2>&1; then
  exec "$(command -v node)" "$ROOT/src/antigravity-bridge.mjs"
fi

for FALLBACK_NODE in "$HOME"/.nvm/versions/node/*/bin/node(N); do
  [[ -x "$FALLBACK_NODE" ]] && exec "$FALLBACK_NODE" "$ROOT/src/antigravity-bridge.mjs"
done
