  ...
  if (githubBuilder) {
    delegatedMcpConfig.mcpServers.github_builder = {
      ...
      env: Object.fromEntries(Object.entries({
        ...
        GITHUB_BUILDER_DELIVERY_PROFILE: githubBuilder.deliveryProfile || null,
        GITHUB_BUILDER_VERIFIED_HEAD_SHA: githubBuilder.verifiedHeadSha || null,
...
${githubReview ? `- After writing the handoff, submit one formal PR review to \`${githubReview.repository}\` PR #${githubReview.prNumber} at \`${githubReview.headSha}\` using \`github_review.submit_pr_review\`. Include a general verdict, a testing-sufficiency summary, and classified inline comments for actionable line-specific findings. Every blocker needs a concrete fix recommendation or suggested patch when practical. The tool is pre-bound to \`${githubReview.expectedLogin}\` and this exact PR head${githubReview.publishStatusGate ? "; it also publishes the exact-head agent-review status" : "; the formal App review is the configured review gate"}.
...
const sharedInput = {
  ...
  githubBuilder: z.object({
    ...
    writerProvider: z.enum(["claude", "codex", "antigravity"]).optional(),
