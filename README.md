...
- The quota footer refreshes independently every 60 seconds. Codex readings come from its local app-server rate-limit API; Claude readings come from Claude Code's zero-turn `/usage` result. Each provider window is shown as remaining percentage, not used percentage. A missing provider window is `—`, a retained reading after a failed refresh is prefixed with `~`, and no value is inferred from model errors or local token counts. Antigravity remains `—` until its CLI exposes account-level quota, while Docker Model Runner and Ollama are labeled `local` because they have no hosted five-hour or weekly allowance. Quota probes are bounded and cancelled during Mission Control shutdown so they cannot freeze the TUI or delay `q`. Use `--no-quota` or `AGENT_BRIDGE_PROVIDER_QUOTA=off` when quota collection must be disabled.
