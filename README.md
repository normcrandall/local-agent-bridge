...
Every portfolio item also carries a versioned predicted footprint: paths, symbols, contracts, exclusive resources, blockers, and the evidence used to infer them. Triage may populate or refine that footprint before dependencies merge, but the implementation frontier still fails closed on unresolved blockers. Moving a lane into an active or integration state atomically reserves its footprint under a repository-scoped lock. The reservation scan includes every active, paused, and integration-pending lane in every portfolio that names the same repository; a stopped or parked lane retains custody until a verified merge or inspected retirement releases it. Explicit conflicts, path/symbol overlap, exclusive resources, and incompatible contract writes are enforced even during the initial shadow-measurement period.
