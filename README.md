...
Governed implementation and publication are separate phases. After implementation commits, the chair records observed verification receipts for that exact clean head. A delivery continuation may enable `ensure_pull_request` only with `githubBuilder.verifiedHeadSha` equal to the receipt-backed repository head and with every requested risk-based command reusable. Adopting or changing the head invalidates publication until fresh receipts exist.
