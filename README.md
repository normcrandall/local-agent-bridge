...
- `bridge cleanup` is always a dry-run unless `--apply` is explicit. Mission Control's history view (`h`) repeats the preview command so cleanup is discoverable from the dashboard. An old terminal collaboration becomes archive-ready only after the configured builder App verifies that its bound pull request is merged/closed or its bound issue is closed. Work-mode collaborations additionally require a clean checkout whose exact head is either the verified merged PR head or the current verified remote branch head. Open delivery, dirty workspaces, unpublished or mismatched heads, unavailable GitHub verification, missing bindings, live workers, active calls, pending coordinator wakes, unacknowledged handoffs, and workspace operations are preserved with explicit reasons. Apply repeats the GitHub and workspace checks immediately before a revision-fenced archive; a changed or reopened outcome skips only that record. Revision-fenced complete portfolios whose items are all terminal remain eligible, and expired native-host receipts and orphaned host lock/temporary files are removed only after their own retention/liveness recheck. Stale `needs_user`, indeterminate, claimed, and nonterminal portfolio work is reported but never auto-cancelled. Archiving preserves JSON/JSONL history and never deletes worktrees, branches, claims, or GitHub history.
