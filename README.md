...
Claimed autonomous work automatically appends all eligible cloud writers to the caller's ordered roster and keeps the selected provider first. If that writer has a confirmed provider-local failure, the broker transfers ownership to the next eligible writer without changing the private checkout or consuming a successful turn. The collaboration ledger, progress narrative, typed failover event, and canonical GitHub issue claim all record the same `from`, `to`, failure class, and reason. Set `providerFailover: { "enabled": false }` only when a lane must remain strictly pinned to the explicitly requested provider; unclaimed work and review-only calls retain their exact caller-supplied roster.
