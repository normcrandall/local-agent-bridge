...
Governed implementation and publication are separate phases. The implementation start binds `push_branch` and `ensure_pull_request` so writer hydration can prove the future publication route, while `workProfile: implement` keeps that route unusable and requires no pre-existing verified head. After implementation commits, the chair records observed verification receipts for that exact clean head. A continuation switches to `workProfile: deliver` and supplies `githubBuilder.verifiedHeadSha` equal to the receipt-backed repository head, with every requested risk-based command reusable. Adopting or changing the head invalidates publication until fresh receipts exist.
