...
- Mission Control is a live terminal dashboard over the same persisted control plane. It defaults to proof-backed live work: a delegated collaboration must have a live worker or a heartbeat no more than 60 seconds old, while a native Claude, Codex, or Antigravity host turn must have a current hook receipt. Native host lanes are labeled separately from delegated writers/reviewers. Terminal or indeterminate state never qualifies merely because it retained an old heartbeat. During a short handoff gap with no proof-backed live lane, the empty live view shows up to three recently ended activities for five minutes and labels the coordinator as potentially between lanes instead of pretending the provider is still running. Mission Control groups all lanes by GitHub repository (including linked worktrees), uses stable human aliases alongside UUIDs, and separately shows process heartbeat, observed output/tool activity, and narrative freshness. It also shows handoff/coordinator state, issue/PR/branch details, merge-train blockers, timing, and a compact event timeline. Its visible timestamps include the local calendar date, time, and timezone alongside relative ages. It never polls GitHub. The `o` and `y` actions use the native macOS URL/clipboard commands, Windows shell helpers, or common Linux desktop commands (`xdg-open` and `wl-copy`/`xclip`/`xsel`) and report a usable fallback when none is installed.
- Run `bridge mc` in a terminal. Use `j`/`k` or the arrow keys to move, `l` for live work, `a` for attention items, `h` for active-ledger history, `s` to switch to attention and reveal collapsed stale records, `o` to open the selected PR, `y` to copy its alias/ID, `c` to continue a stopped compatible collaboration, `x` to cancel, `A` to archive, `w` to acknowledge a processed coordinator wake, and `q` to exit. Cancel, archive, and wake acknowledgement require a second confirming keypress; continue/cancel/archive are fenced to the rendered `updatedAt`, and acknowledgement is fenced to the wake sequence. Exit waits until raw mode, cursor visibility, and the alternate screen have been restored before returning control to the shell. Use `bridge mc --snapshot` for logs and scripts, `bridge mc --json` for machine-readable state, `bridge mc --attention` for attention items, `bridge mc --repo OWNER/REPO` to focus that exact repository, `bridge mc --include-stale` to reveal old unresolved records, and `bridge mc --all` to include terminal history. Non-interactive output automatically uses snapshot mode.
