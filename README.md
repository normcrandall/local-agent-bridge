...
Before a writer provider launches, the broker runs the approved `postCreate` recipe and records a durable hydration receipt. Hydration proves checkout and private-index writes, commit-object and scratch-ref creation, origin stability, and either a bound builder-App route or configured remote handoff; the scratch ref and probe file are removed before implementation begins. An interrupted checkout retains its receipt so restart recovery diagnoses the exact hydration stage instead of silently reusing ambiguous work.
