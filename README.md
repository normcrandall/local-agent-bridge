...
- Run `bridge mc` in a terminal. Use `Tab` to cycle panes on a narrow terminal, `Enter` to expand or collapse selected-lane details, `j`/`k` or the arrow keys to move, `l` for live work, `a` for attention items, `h` for active-ledger history, `s` to switch to attention and reveal collapsed stale records, `o` to open the selected PR, `y` to copy its alias/ID, `c` to continue a stopped compatible collaboration, `x` to cancel, `A` to archive, `w` to acknowledge a processed coordinator wake, and `q` to exit. Changing the view or selection clears old action messages and confirmations. Cancel, archive, and wake acknowledgement require a second confirming keypress; continue/cancel/archive are fenced to the rendered `updatedAt`, and acknowledgement is fenced to the wake sequence. Exit waits until raw mode, cursor visibility, and the alternate screen have been restored before returning control to the shell. Use `bridge mc --snapshot` for logs and scripts, `bridge mc --json` for machine-readable state, `bridge mc --attention` for attention items, `bridge mc --repo OWNER/REPO` to focus that exact repository, `bridge mc --include-stale` to reveal old unresolved records, and `bridge mc --all` to include terminal history. Non-interactive output automatically uses snapshot mode.
