...
- The quota footer refreshes independently every 60 seconds. Codex readings come from its local app-server rate-limit API; Claude readings come from Claude Code's non-persistent, safe-mode `/usage` result and are accepted only when Claude reports that it used zero inference turns. Each provider window is shown as remaining percentage, not used percentage. A missing provider window is `—`; a last-good reading is prefixed with `~` after a failed refresh and expires after five minutes; no value is inferred from model errors or local token counts. Antigravity remains `—` until its CLI exposes account-level quota, while Docker Model Runner and Ollama are labeled `local` as a provider category, not as a liveness assertion. Quota probes are bounded and cancelled during Mission Control shutdown so they cannot freeze the TUI or delay `q`. One-shot `--snapshot` and `--json` output do not spawn provider CLIs by default; add `--quota` when a synchronous quota reading is wanted. Use `--no-quota` or `AGENT_BRIDGE_PROVIDER_QUOTA=off` when interactive quota collection must be disabled.
