...
The installer also adds native-host activity hooks for Claude, Codex, and Antigravity. Prompt submission/agent-start records a bounded local task label, tool hooks refresh the heartbeat and narrative, and stop/session-end clears the lane. Codex parent and subagent turns are tracked as a hashed active set: a `SubagentStop` removes only that turn, while the authoritative root `Stop` clears the session. Session and turn identifiers are hashed before storage and the receipts live only under `~/.local/share/agent-bridge/state/host-activity`; no raw identifier is persisted. Native lanes prefer a live parent host process as proof, with a two-minute recent-receipt fallback for hosts that insert a short-lived shell before the hook; the one-hour receipt TTL remains a secondary bound for long non-tool inference. A crashed host therefore disappears within two minutes even if its parent PID cannot be proven. Hooks without a session identifier fail closed instead of collapsing concurrent sessions into one workspace lane. Expired receipts are pruned when a new native turn starts and by `bridge cleanup --apply`. Restart each active host after installation so it loads the new hooks; Codex may ask you to trust the changed hook configuration. See the [Codex hook reference](https://developers.openai.com/codex/hooks) and [Gemini CLI hook reference](https://geminicli.com/docs/hooks/reference/) for the underlying lifecycle events.
...
The adapter uses Docker's documented Ollama-compatible loopback API at `http://127.0.0.1:12434`, specifically the currently documented `/api/tags` and `/api/chat` surfaces. This is a compatibility dependency, not a bridge-owned or version-negotiated Docker API contract, so the contract test checks both response shapes and verifies that `/api/chat` preserves a requested tool call. The live probe is deliberately opt-in (`AGENT_BRIDGE_LIVE_DOCKER_CONTRACT=1 npm run test:docker`) so the normal suite remains hermetic. `DOCKER_MODEL_RUNNER_MODEL` and `DOCKER_MODEL_RUNNER_HOST` are explicit machine overrides. Non-loopback endpoints are rejected because Docker Model Runner's local API is unauthenticated. Use `agents: ["docker"]` for local review. A mixed `docker`/`ollama` roster is safe because Ollama preflight reports unavailable while the selected Docker reviewer route is healthy. The MCP exposes `ask_docker`, durable `continue_docker`, and `get_docker_status`; state is stored owner-only under `~/.local/state/local-agent-bridge/docker-sessions`.
...
Work-mode delivery is explicit and fail closed. When builder and reviewer Apps are configured, the machine default is `github-governed`: start with an `issueTarget` or `issueClaim` and a self-contained checkout. The broker hydrates the issue and derives the initial minimal writer-App binding from the selected writer, checkout branch/base, and exact starting head; callers do not copy the issue body into task prose or manually construct `githubBuilder`. Publication remains PR-only. For intentionally unpublished work, pass `deliveryProfile: "local-only"`; that profile rejects issue targets, claims, and builders before any GitHub hydration. Native `merge_pull_request` calls include the exact target-repository workspace so its repository policy controls authorization. The broker persists the successful exact-head merge and issue-comment receipt; `record_portfolio_merge` consumes that receipt instead of trusting caller-attested completion.

Governed implementation and publication are separate phases. The implementation start derives or validates `push_branch` and `ensure_pull_request` so writer hydration can prove the future publication route, while `workProfile: implement` makes the broker retain and withhold that route from the provider and requires no pre-existing verified head. After implementation commits, the chair records observed verification receipts for that exact clean head. A continuation switches to `workProfile: deliver` and supplies `githubBuilder.verifiedHeadSha` equal to the receipt-backed repository head, making the bound route available with every requested risk-based command reusable. Adopting or changing the head invalidates publication until fresh receipts exist.

Durable repository publication uses a checkpointed journal outbox. Retention preserves every unpublished, leased, or retrying item and compacts live state only from a complete checkpoint floor; it never keeps an orphan lifecycle suffix. Acknowledged and dead-letter items age out after the configured `terminalHorizonMs` (30 days by default). Every retention receipt reports total terminal eviction in `droppedTerminalItems` and the dead-letter subset in `droppedDeadLetterItems`, while a protected live set larger than `maxRecords` fails closed with `RETENTION_FLOOR_EXCEEDED`. Because the hash-linked journal can only retain a contiguous suffix, discarding terminal outbox history also removes every earlier record regardless of payload type; callers must therefore give the publication outbox its own journal rather than mix unrelated general-journal records into that file.

Before a writer provider launches, the broker runs the approved `postCreate` recipe and records a durable hydration receipt. Hydration proves checkout and private-index writes, commit-object and scratch-ref creation, and origin stability. A bound builder App is accepted only when it authorizes both branch publication and pull-request creation; a configured remote handoff remains usable for local implementation but is explicitly recorded as configured and unproven rather than as authorization. Scratch refs and current or stale probe files are removed before implementation begins. An interrupted checkout retains its receipt so restart recovery diagnoses the exact hydration stage instead of silently reusing ambiguous work.
...
Claim authority is resolved before the first GitHub mutation. The durable lease stores the canonical `<app-slug>[bot]` login separately from the verified App ID, installation ID, repository, and granted permission set; installation tokens and private-key material are never persisted. A configured login may omit the `[bot]` suffix, but the broker canonicalizes it only at App credential resolution and continues to use exact bot-versus-human checks elsewhere. `rebind_issue_claim` repairs display-login drift only when every stored authority identifier and the active collaboration still match. A legacy claim without those identifiers, or a claim whose App, installation, or repository changed, fails with a repairable diagnosis: inspect and use `release_issue_claim`, then reacquire before any further mutation.

Restart claim reconciliation is isolated per portfolio item. A transient GitHub or transport failure retains the current lease and is retried by a later sweep; an authority mismatch or durable local-state failure marks only that lane indeterminate. Unreadable or structurally invalid portfolio files are never guessed at or overwritten: the sweep reports and skips them while continuing with independently readable portfolios. If even the local failure diagnosis cannot be persisted, the retained GitHub claim remains the fail-closed ownership fence and the sweep reports that secondary write failure without preventing later lanes from reconciling.
...
Every provider-specific reviewer App submits a formal review at the exact authorized head. Actionable inline findings are stamped with that head and reviewer identity. The writer must answer each one as fixed, declined with rationale, or converted to a linked follow-up issue; those responses are stamped with the refreshed exact head. Only the App that opened a thread may resolve it after approving and verifying that refreshed head. Foreign, unanswered, and reopened actionable threads remain merge blockers, while a PAT compatibility comment never enters the state machine.

When the optional status permission is present, `REQUEST_CHANGES` publishes `agent-review=failure` and `COMMENT` publishes pending. An `APPROVE` remains pending while actionable threads are unanswered or unresolved, then advances to success after the owning reviewers resolve their verified threads. A head change expires the prior approval, writer dispositions, and merge-readiness receipt. The bound builder re-reads the live thread state immediately before merge and accepts only an exact-head formal App approval, trusted `agent-review=success` fallback, or exact-head approval from a configured `mergePolicy.trustedHumanReviewers` login together with a clean thread receipt. GitHub still enforces CI and the ruleset. If a repository keeps a nonzero human approval count, the pipeline pauses until the person actually reviews; it never manufactures that approval through an owner PAT or administrator bypass.
...
- `goal-loop`: build toward verified completion through bounded, resumable council cycles; when composed by `take-the-helm`, each bounded leg returns to the autonomous portfolio scheduler instead of prompting the user to continue.
- `pair-program`: rotate implementation and review roles with preflight, worktrees, visible progress, recovery, CI, budgets, and automatic exact-head GitHub review after governed delivery.
...
- `take-the-helm`: give the council operational ownership of a goal or queue, schedule independent issues into parallel worktree lanes, and serialize integration through a bridge-owned merge train. It automatically advances every governed lane through delivery, formal non-writer PR review, repair/re-review, authorized merge, cleanup, and next-frontier dispatch; provider turns and bounded cycles never require the user to say "continue". It self-sequences new work (review follow-ups, `council-wayfinder` decomposition output), decomposes oversized items via `council-wayfinder`, applies a two-tier review circuit-breaker (5-round follow-up → extracted ticket, 12-round → council disposition), runs in wave/phase/end-to-end modes, and requires reviewers to include a proposed fix.
...
`needs_user` becomes a machine-level attention signal only when the provider has actually stopped and the collaboration has a fresh, unacknowledged `needs_user` coordinator wake. A bare legacy status, an active provider that is still resolving the question, a portfolio-only state, or an old/acknowledged wake remains inspectable without ringing or inflating the red current-input banner. Each immutable request alerts once; a changed wake sequence can alert again, while failed delivery retries with bounded exponential backoff. The notification identifies the repository and bridge, and on macOS its click action opens a Terminal window directly into repository-filtered Mission Control. Mission Control repeats the repository, short bridge ID, and protected-decision summary in the red banner so the attention view identifies the exact request. Use `bridge attention list` to inspect current stopped-provider requests; requests older than the alert window remain in its `historical` array rather than disappearing. Add `--repo OWNER/REPO` to filter either group. Use `bridge attention notify` to retry an undelivered current request and `bridge attention test --repo OWNER/REPO` to verify the desktop notification path. Set `AGENT_BRIDGE_ATTENTION_NOTIFICATIONS=off` only when desktop alerts must be disabled; durable state and attention rows remain available without notification writes. Set `AGENT_BRIDGE_ATTENTION_DETAIL=generic` to hide repository and bridge names from the lock-screen preview without disabling click-through.
...
| `replay-collaboration` | Replay incident records and identify next safe actions. |
| `goal-loop` | Build toward explicit completion criteria through bounded plan, implement, review, fix, and verification cycles; hand composed legs back to the helm scheduler automatically. |
| `pair-program` | Rotate one writer and independent reviewers across tasks, worktrees, CI, and automatic exact-head formal PR reviews. |
...
| `take-the-helm` | Autonomously schedule safe parallel issue lanes and drive delivery, exact-head review, repair, authorized merge, cleanup, and next-frontier dispatch without continue prompts, while preserving narrow escalation boundaries. Self-sequences new work, decomposes oversized items via council-wayfinder, applies a two-tier review circuit-breaker, and supports explicit wave/phase or default end-to-end modes. |
...
- `plan_portfolio` / `create_portfolio`: validate a dependency and conflict graph, compute safe execution waves plus up to two triage-ahead candidates, and create a durable `helm-<uuid>` ledger.
- `get_portfolio` / `list_portfolios` / `update_portfolio_item`: inspect and update revision-controlled issue lanes across host apps.
- `inspect_repository_footprints` / `reconcile_portfolio_footprint`: inspect repository-wide reservations and reconcile committed, staged, unstaged, and untracked actual paths against the versioned predicted footprint during work and immediately before publication.
...
`take-the-helm` uses a durable portfolio ledger under `~/.local/share/agent-bridge/state/portfolios`. Each issue declares hard blockers, temporary conflict edges, expected path ownership, exclusive resources, priority, phase order, and verification commands. `plan_portfolio` rejects dependency cycles and greedily fills the highest-priority non-conflicting frontier up to `maxParallel`, whose fixed default is five to match the out-of-box writer policy; an operator may lower it to match a stricter machine configuration. A phase is an ordering and reporting boundary rather than a concurrency fence: callers that use named phases should also supply numeric `phaseOrder`, because phase names alone intentionally share the default order `0`. When the current phase cannot fill capacity, dependency-ready later-phase items may be selected as explicit lookahead lanes, but never through a dependency, claim, footprint, resource, checkout, review, CI, or merge gate. A selected implementation issue receives one writer and one self-contained checkout under `.bridge/writer-checkouts`; its Git index, refs, and objects do not alias the source repository. Review-only lanes may still use linked worktrees under `.bridge/worktrees`. Provider execution uses role-specific semaphores: the out-of-box machine default is five live work calls and ten concurrent read-only review calls per provider. Excess calls remain queued in FIFO order with `runtime.activeCall.phase: waiting_capacity` and start automatically when a compatible slot is released.

The helm scheduler treats provider completion as an event rather than a stopping point. A governed writer handoff is verified and delivered to a PR, then immediately dispatched to an eligible non-writer with `githubReview` bound to the exact remote head. Valid blocking findings return to the same writer and trigger a fresh exact-head review. Approval plus required CI advances through the merge train when repository policy authorizes it; merge completion releases the claim and checkout and backfills the next safe lane. The overall scheduler stops only at an explicit wave/phase boundary, a completed objective, user pause or cancellation, portfolio budget, or when every remaining lane is at a protected `needs_user`/`indeterminate` boundary or hard escalation. "No ready work" is terminal only when no delivery, review, repair, CI, merge, recovery, or queued-provider event remains. Provider turn limits and bounded goal-loop cycles restart or recover their leg without asking the user to continue.

Every portfolio item also carries a versioned predicted footprint: paths, symbols, contracts, exclusive resources, blockers, and the evidence used to infer them. Triage may populate or refine that footprint before dependencies merge, but the implementation frontier still fails closed on unresolved blockers. Moving a lane into an active or integration state atomically reserves its footprint under a repository-scoped lock. `create_portfolio` records an explicit `OWNER/REPO` identity or derives it from the GitHub origin, so separate worktree paths cannot split the lock domain. The enforcement scan includes every active and integration-pending reservation in every portfolio for that repository. A conflict-parked lane retains its reservation identity and remains inspectable without blocking other work, but must pass the complete current conflict scan before it can resume. Explicit duplicates, conflict edges, path/symbol overlap, exclusive resources, and incompatible contract writes are enforced even during the initial shadow-measurement period.

Call `reconcile_portfolio_footprint` during implementation and again before publication. The bridge captures committed changes from the portfolio target through the lane head plus staged, unstaged, and untracked paths, then records prediction precision/recall and unexpected scope. If newly observed scope creates a hard conflict, the newer or lower-priority lane is parked without deleting its checkout or reservation; the older/higher-priority lane continues. `inspect_repository_footprints` exposes current and stale reservations so restart recovery cannot silently duplicate work. Shadow-mode accuracy receipts are intended to tune footprint prediction before increasing ordinary writer concurrency; they never weaken deterministic conflict enforcement.
...
Each participant receives a redacted, size-bounded full task capsule only on its first turn. GitHub-governed resumptions persist a repository-journal cursor per provider and carry only checksum-verified unseen records plus the compact peer reply. Cursor gaps, changed bindings, corrupt cursors, unverifiable journals, and oversized records force a bounded full resync and record a typed `repository_context_resync_receipt`; skipped record sequence numbers remain in the audit trail without exposing their rejected content. Runtime `promptMetrics` reports full, delta, and resync prompts, UTF-8 bytes sent, estimated tokens, and avoided repeated bytes as well as the legacy character counters.
...
Claimed autonomous work automatically preflights all eligible cloud writers but keeps appended providers on standby. They do not consume turns, budget, or capacity until the active writer has a confirmed provider-local failure. The broker then transfers ownership to the next eligible writer without changing the private checkout or consuming a successful turn. The collaboration ledger, progress narrative, typed failover event, and canonical GitHub issue claim all record the same `from`, `to`, failure class, and reason. Set `providerFailover: { "enabled": false }` only when a lane must remain strictly pinned to the explicitly requested provider; unclaimed work and review-only calls retain their exact caller-supplied roster.

An indeterminate timeout or lost execution boundary never transfers ownership. When no writer remains, the terminal error lists every attempted provider, failure class, and original reason instead of reporting a generic model-capacity error. Only exhaustion in which every provider failed for transient model capacity enters visible `recovering` state and retries according to `providerRecovery` (three attempts at 15, 60, and 180 seconds by default). Mixed failures such as overload plus configuration, authentication, permission, quota, policy, invalid request, or transport failure stop with their exact aggregate diagnosis and do not enter a misleading capacity loop.

Existing collaborations retain their recorded workspace and failover policy across restarts and continuations. To rescue an older stopped work collaboration from a linked worktree, inspect its exact workspace and HEAD, call `recover_writer_checkout` with both fences, and then continue the same collaboration. Recovery rebuilds the lane from its recorded base, migrates committed, staged, deleted, modified, and untracked changes into private Git custody, and retains the original linked worktree as evidence. `cleanup_writer_checkout` removes a stopped private checkout only after exact workspace and HEAD inspection; it refuses dirty state unless `discardChanges: true` is explicit. Both operations atomically reserve the collaboration as `indeterminate` before touching the filesystem, so a concurrent continuation cannot start a worker in a checkout being moved or removed; an interrupted operation leaves a durable reconciliation marker instead of guessing ownership. `wait_for_portfolio_lane` races the desired head advancement against handoff, failure, cancellation, indeterminate ownership, and recovery so coordinators cannot park on a success-only signal.
...
bridge recover --all # fleet-wide dry-run recovery classification
bridge wait <bridge-id-or-alias> [more...] --any --after-updated-at 2026-07-24T11:45:00Z --timeout 3600
bridge archive bridge-<uuid>
bridge prune --older-than-days 30
bridge cleanup --older-than-days 7 # dry-run; add --apply to archive safe records
...
- Mission Control is a live terminal dashboard over the same persisted control plane. Its default operator view is one fixed, full-height grid: repositories on the left, deduplicated work in the center, and details for the selected lane on the right. When work is live, the default view scopes non-live waits and stopped attempts to the repository or repositories currently executing; fresh protected `needs_user` boundaries remain globally visible. The repository pane reports how many rows and repositories are outside that live scope, and JSON exposes the same `scopedOut` summary. Standalone terminal collaboration receipts such as `turn_limit`, `agreed`, and `cancelled` are history rather than WAITING, while an open portfolio item may remain a genuine wait after its provider turn ends. A cancelled attempt for open work is STOPPED, never WAITING, and missing portfolio status cannot revive a terminal receipt. Use attention or history when intentionally inspecting the broader ledger. Blocked work includes a dedicated `BLOCKED BECAUSE` explanation. It leads with any broker/provider error or pending decision, then shows only unresolved portfolio dependencies with their titles when available, so actionable context survives narrow layouts. Delivered work never renders a blocker even when its declared dependency metadata remains. If the coordinator persisted only a bare blocked state, Mission Control says that no reason was recorded instead of presenting `blocked` as though it were an explanation. Every row is measured by terminal display width, so ANSI color and Unicode cannot move a divider; narrow terminals collapse to one pane rather than damaging the grid. Semantic color distinguishes active work, stopped provider attempts, user boundaries, waiting work, selection, and stale narrative, while labels and symbols preserve the same meaning under `NO_COLOR` or `--no-color`. `STOPPED` means an execution attempt ended because of provider failure, budget, cancellation, or uncertain ownership; it does not claim that the issue or pull request failed. Persisted terminal portfolio outcomes supersede non-actionable attempt state, so merged/completed work leaves the live `STOPPED` count and appears in history with separate `DELIVERY` and `ATTEMPT` evidence; a still-live provider or unresolved user boundary remains selectable until the attempt actually stops. Repeated issue/PR identities become one operator row with a related-lane count, repetitive provider-tool messages are suppressed, and consecutive identical timeline events are coalesced. JSON snapshots expose `stopped` while retaining `failed` count and `legacyOperatorCategory` aliases for older consumers. Full UUIDs, workspace paths, timing, and provider activity stay behind detail expansion instead of dominating the default screen.
- Interactive Mission Control redraws immediately when subscribed control-plane events arrive. `--refresh-ms N` separately sets the maximum idle redraw heartbeat from 100 through 3,600,000 milliseconds (default 60,000); malformed or out-of-range values fail instead of being silently clamped.
- Proof-backed liveness remains unchanged: a delegated collaboration must have a live worker or a heartbeat no more than 60 seconds old, while a native Claude, Codex, or Antigravity host turn must have a current hook receipt. Terminal or indeterminate state never qualifies merely because it retained an old heartbeat. During a short handoff gap with no proof-backed live lane, the selected-lane pane shows recently ended activity and labels the coordinator as potentially between lanes instead of pretending the provider is still running. Mission Control never polls GitHub. The `o` and `y` actions use the native macOS URL/clipboard commands, Windows shell helpers, or common Linux desktop commands (`xdg-open` and `wl-copy`/`xclip`/`xsel`) and report a usable fallback when none is installed.
- Run `bridge mc` in a terminal. `Tab` and the right arrow move focus forward across all three panes; `Shift-Tab` and the left arrow move backward. In **Repositories**, `j`/`k` or up/down selects a repository and filters the work list; `Enter` moves into **Work**. In **Work**, the same keys select a lane and `Enter` moves into **Details**. In **Details**, they scroll the selected lane's record, `g`/`G` jump to its beginning/end, and `Enter` expands or collapses the full detail. The footer always names the focused pane and its available navigation. Use `l` for live work, `a` for attention items, `h` for active-ledger history, `s` to switch to attention and reveal collapsed stale records, `r` to clear the supervisor's repository lookup cache and visibly refresh the subscribed projection, `o` to open the selected PR, `y` to copy its alias/ID, `c` to continue a stopped compatible collaboration, `x` to cancel, `A` to archive, `w` to acknowledge a processed coordinator wake, and `q` to exit. View changes preserve an explicit repository selection while that repository remains present. Changing the lane selection clears old action messages and confirmations. A deduplicated `+N` row acts only on its displayed representative and the confirmation names how many related lanes remain. Cancel, archive, and wake acknowledgement require a second confirming keypress; continue/cancel/archive are fenced to the rendered `updatedAt`, and acknowledgement is fenced to the wake sequence. Exit waits until raw mode, cursor visibility, and the alternate screen have been restored before returning control to the shell. Use `bridge mc --snapshot` for a bounded expanded record suitable for logs, `bridge mc --json` for machine-readable state, `bridge mc --attention` for attention items, `bridge mc --repo OWNER/REPO` to lock the dashboard to that exact repository, `bridge mc --include-stale` to reveal old unresolved records, and `bridge mc --all` to include terminal history. Non-interactive output automatically uses snapshot mode.
- The quota band refreshes independently every 60 seconds. Wide terminals use two rows: a refresh/check timestamp followed by color-coded Codex, Claude, and Antigravity groups; compact terminals collapse that information to one bounded row. Remaining percentages are green at 50% or more, yellow below 50%, and red below 20%, while provider names retain distinct colors. `NO_COLOR` and `--no-color` preserve the same labels without ANSI styling. Codex readings come from its local app-server rate-limit API, which may report only the weekly window; that case is labeled `not reported`, not treated as a failed probe. Claude readings come from Claude Code's non-persistent, safe-mode `/usage` result and are accepted only when Claude reports that it used zero inference turns. A failed provider probe is `unavailable`; an initial probe is `loading`; a last-good reading is prefixed with `~` after a failed refresh and expires after five minutes. Antigravity is `not exposed` until its CLI offers account-level quota. Docker Model Runner and Ollama are intentionally omitted because local review runtimes do not have hosted account quota. No value is inferred from model errors or local token counts. Quota probes are bounded and cancelled during Mission Control shutdown so they cannot freeze the TUI or delay `q`. One-shot `--snapshot` and `--json` output do not spawn provider CLIs by default; add `--quota` when a synchronous quota reading is wanted. Use `--no-quota` or `AGENT_BRIDGE_PROVIDER_QUOTA=off` when interactive quota collection must be disabled.
- `bridge cleanup` is always a dry-run unless `--apply` is explicit. Mission Control's history view (`h`) repeats the preview command so cleanup is discoverable from the dashboard. An old terminal collaboration becomes archive-ready only after read-only GitHub API calls through the configured App verify that its bound pull request is merged/closed or its bound issue is closed. A closed-unmerged PR also requires its exact head to remain recoverable from the verified remote branch. Work-mode collaborations additionally require a clean checkout rooted at the recorded repository whose exact head is either the verified merged PR head or the current verified remote branch head. Open delivery, dirty workspaces, unpublished or mismatched heads, unavailable GitHub verification, missing GitHub bindings or targets, live workers, active calls, pending coordinator wakes, unacknowledged handoffs, and workspace operations are preserved with explicit reasons. Old terminal records without a GitHub binding remain visible and are never auto-archived. Apply repeats the GitHub and workspace checks immediately before a revision-fenced archive; a changed or reopened outcome skips only that record. Revision-fenced complete portfolios whose items are all terminal remain eligible, and expired native-host receipts and orphaned host lock/temporary files are removed only after their own retention/liveness recheck. Stale `needs_user`, indeterminate, claimed, and nonterminal portfolio work is reported but never auto-cancelled. Archiving preserves JSON/JSONL history and never deletes worktrees, branches, claims, or GitHub history.

### Repository journal operations

Use `bridge journal` for operator-safe repository journal maintenance. The default directory is `.bridge/repository-journal`; select another store with `--directory PATH`.

- `bridge journal inspect` reports chain health, repository binding, sequence/digest bounds, and protected states.
- `bridge journal retain --max-records N` previews the exact retained and removed records. Add `--apply` only after inspection. The apply pass rechecks the source, creates a checksummed recovery bundle, and atomically swaps the journal. Pending, leased, backoff, dead-letter, dirty, unpublished, or indeterminate work is preserved or the operation fails closed.
- `bridge journal archive [--output FILE]` writes a verified, non-destructive archive bundle and leaves the live journal untouched.
- `bridge journal export --output FILE --repository OWNER/REPO` creates a checksummed, binding-aware portable bundle. `bridge journal import --input FILE --repository OWNER/REPO` validates it without changing the target; add `--apply` to atomically restore it after a recovery receipt is written.
- `bridge journal recover` previews a torn or corrupt suffix and the valid prefix. Add `--apply` to archive every original byte in a checksummed recovery envelope before restoring only the verified prefix.
- `bridge journal restore --input RECEIPT` verifies a corruption recovery envelope and previews its exact raw-byte restoration. Add `--apply` to restore those bytes atomically.

Every destructive command is a dry-run unless `--apply` is explicit. Successful applies return `recoveryReceipt.path`, its checksum, the affected sequence range, and an exact `restoreCommand`. An interrupted retain, import, or corruption repair rolls the live file back before releasing the journal lock.
...
- Workspace recipes live at `.agent-bridge/workspace-recipes.json`, but execute only when the exact phase command list also appears under that absolute workspace in `~/.config/local-agent-bridge/workspace-recipe-approvals.json`. `postCreate`, `prePublish`, and `preRetire` are supported; approved `postCreate` runs before a new delegated writer starts. Repository delivery policy may additionally name command sets for the `quick`, `full`, `prePublish`, `integration`, and `preRetire` verification roles without embedding commands in broker code; pass `verificationRole` to `start_collaboration` or `continue_collaboration` to resolve and execute that role, while retirement requires the named `preRetire` role to match the approved recipe exactly. Preview recipes with `bridge workspace recipe`; add `--apply` for an explicitly approved phase. Recipes run in a non-login POSIX shell with a bounded timeout and a small environment allowlist; broker credentials, tokens, and private-key paths are not inherited, copied, or linked.

- `retire_writer_checkout` is the only writer-removal path. It refuses active, dirty, moved-remote, unpublished, open-PR, or unrecoverable work. After a merge it records exact writer/merge SHA recovery, runs the approved `preRetire` recipe, rechecks that the checkout remains clean, and removes the private checkout. Mutating a user's source checkout is off by default; a repository may opt in with `retirement.updateLocalDefaultBranch: true`, in which case the bridge records the previous SHA before mutation and refuses a default branch checked out in any linked worktree. The receipt persists the cleaned path, branch disposition, merged SHA, and recovery source. Every stage is durable; deterministic failures restore the prior collaboration state for a corrected retry, while a missing checkout remains `indeterminate` for explicit reconciliation.
...
```json
{
  ...
  "modelFallbacks": {
    ...
    "codex": ["gpt-5.6-terra"],
    "antigravity": ["gemini-3.6-flash-medium", "gemini-3.6-flash-low", "gemini-3.5-flash-high"],
...
The global installer fills missing Codex and Antigravity writer chains in `~/.config/local-agent-bridge/model-fallbacks.json` while preserving every existing provider entry. An explicit empty provider array remains a machine-wide opt-out. `AGENT_BRIDGE_MODEL_FALLBACKS_CONFIG` redirects both runtime reads and installer writes to the same custom path. Copy [`config/model-fallbacks.example.json`](config/model-fallbacks.example.json) when you want to configure the complete provider roster manually. Configured primary models remain unchanged. Claude Code receives its ordered chain through the native `--fallback-model` option. After an explicit Codex overload, a new delegated turn retries from a fresh thread with the original task, while `codex-reply` retains the caller's established thread; both forms repeat the original prompt and tell the fallback to preserve completed workspace work. Passing a provider's `[]` disables the machine policy for one collaboration. The total request bound reserves one inactivity window per configured attempt, so a three-model Antigravity fallback chain can take up to four per-attempt windows only when each prior model stays alive until its inactivity limit.
...
Writer-side PR delivery uses a separate `githubBuilder` authorization bound to one repository, expected bot login, feature ref, exact base, optional PR, and an explicit `allowedOperations` list. Claude and Codex receive only `github_builder` tools; Antigravity returns a validated operation envelope that the broker executes outside model context. An intermediate Antigravity turn may return an explicit empty operation batch without being marked unavailable; a terminal writer handoff without a remotely verified builder receipt is recorded as rejected delivery and wakes the coordinator. For a self-contained writer checkout, the broker may adopt only that checkout's current `HEAD` after verifying self-contained Git metadata, the fixed local ref, exact origin, base ancestry, and payload; caller input is clamped off outside that checkout type. The immutable authorization head and the active delivery head remain distinct in durable receipts, including an explicit head-transition record. This lets the writer create a commit that did not exist when the lane was authorized without granting an arbitrary SHA or branch. Supported actions are create/update the designated PR, read/reply/resolve exact review threads, mark ready, merge the exact PR at the exact active head SHA, create a bound feature branch, fast-forward a bound feature branch, and replace a bot-owned feature-branch head with an exact old/new SHA lease. Every mutation rechecks the per-operation allowlist, identity, repository/ref/head binding, protected/default-branch state, and payload constraints, then returns a remotely verified durable receipt. Branch replacement is separately allowlisted as `replace_branch`; it is unavailable by default and never exposes raw force-push access. Before merge, the builder requires an exact-head approval from a configured reviewer App, `agent-review=success` from that App, or an exact-head approval from a configured trusted human. The default allowlist excludes `merge` and every branch mutation; normal goal-loop and pair-program runs stop at a green reviewed PR unless the user explicitly adds the required exact-head operation.
...
- An explicit `permissionProfile: yolo` remains available for work mode and maps to Claude Code permission bypass, Codex `danger-full-access` with approvals disabled, and Antigravity auto-approval without its terminal sandbox. Additionally, an Antigravity review carrying `verificationCommands` automatically uses `--dangerously-skip-permissions`: `agy` exposes no exact non-interactive command grant, and the owner-selected policy prefers a working command-running reviewer over removing Antigravity from the roster. Static Antigravity reviews remain sandboxed. Claude keeps exact command grants. Codex, Docker, and Ollama automatically continue as static exact-head reviewers with unsupported commands withheld; local/full and hosted CI stay separate evidence, and the lower-level command boundary remains fail-closed.
