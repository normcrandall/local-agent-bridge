...
- `bridge cleanup` is always a dry-run unless `--apply` is explicit. It archives only old terminal collaborations without a live worker, active call, pending coordinator wake, unacknowledged handoff, or workspace operation, plus revision-fenced complete portfolios whose items are all terminal. Stale `needs_user`, indeterminate, claimed, and nonterminal portfolio work is reported but never auto-cancelled. Every candidate is rechecked under its update lock; a concurrent change skips that record without aborting the rest of the batch. Archiving preserves JSON/JSONL history and never deletes worktrees, branches, claims, or GitHub history.
