...
`needs_user` is also a machine-level attention signal, not only a stored status. The broker immediately sends a desktop notification with sound for a chaired wake; the persistent supervisor discovers chair-less `needs_user` states within 60 seconds and repeats every fresh pending alert every 15 minutes until the wake is acknowledged or the collaboration advances. A request is fresh for six hours by default, based on its immutable wake, handoff/escalation, or collaboration creation time—not later notification-receipt writes. Older requests remain inspectable in Mission Control and `bridge attention list`, but do not ring, notify, or inflate the red current-input banner. Delivery is deduplicated and recorded on the wake, or directly on a chair-less collaboration; failed adapters retry with bounded exponential backoff. Notification previews are generic and keep repository names and protected decision details inside the durable receipt; set `AGENT_BRIDGE_ATTENTION_DETAIL=repository` only when repository names are safe to expose in desktop previews. Mission Control shows a red `USER INPUT REQUIRED` banner—even in live mode—and rings the terminal bell once for each newly observed fresh request. Use `bridge attention list` to inspect pending requests, `bridge attention notify` to force a manual resend, and `bridge attention test` to verify the desktop notification path. Set `AGENT_BRIDGE_ATTENTION_NOTIFICATIONS=off` only when desktop alerts must be disabled; durable state and Mission Control warnings remain active without periodic notification writes.
