...
- Mission Control is a live, read-only terminal dashboard over the same persisted control plane. It defaults to proof-backed live work, groups collaborations and portfolio lanes by GitHub repository (including linked worktrees), shows the active provider and role, narrative freshness versus heartbeat freshness, handoff/coordinator state, issue/PR/branch details, merge-train blockers, timing, and a compact event timeline. It never starts providers or polls GitHub.
- Run `bridge mc` in a terminal. Use `j`/`k` or the arrow keys to move, `l` for live work, `a` for attention items, `h` for active-ledger history, `s` to reveal collapsed stale records, `r` to refresh, and `q` to exit. Use `bridge mc --snapshot` for logs and scripts, `bridge mc --json` for machine-readable state, `bridge mc --attention` for attention items, `bridge mc --repo OWNER/REPO` to focus that exact repository, `bridge mc --include-stale` to reveal old unresolved records, and `bridge mc --all` to include terminal history. Non-interactive output automatically uses snapshot mode.
- `bridge cleanup` is always a dry-run unless `--apply` is explicit. It archives only old terminal collaborations without a live worker, active call, pending coordinator wake, unacknowledged handoff, or workspace operation, plus complete portfolios whose items are all terminal. Stale `needs_user`, indeterminate, claimed, and nonterminal portfolio work is reported but never auto-cancelled. Archiving preserves JSON/JSONL history and never deletes worktrees, branches, claims, or GitHub history.
