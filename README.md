...
`needs_user` is also a machine-level attention signal, not only a stored status. The broker immediately sends a desktop notification with sound, and the persistent supervisor repeats it every 15 minutes until the wake is acknowledged. Delivery is deduplicated and recorded on the wake; a failed desktop attempt remains retryable. Notification previews name only the repository or workspace and keep protected decision details inside the durable receipt. Mission Control always shows a red `USER INPUT REQUIRED` banner—even in live mode—and rings the terminal bell once for each newly observed wake. Use `bridge attention list` to inspect pending requests, `bridge attention notify` to resend them, and `bridge attention test` to verify the desktop notification path. Set `AGENT_BRIDGE_ATTENTION_NOTIFICATIONS=off` only when desktop alerts must be disabled; durable state and Mission Control warnings remain active.
