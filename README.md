...
- `bridge cleanup` is always a dry-run unless `--apply` is explicit. Mission Control's history view (`h`) repeats the preview command so cleanup is discoverable from the dashboard. It archives only old terminal collaborations without a live worker, active call, pending coordinator wake, unacknowledged handoff, or workspace operation, plus revision-fenced complete portfolios whose items are all terminal. It also removes expired native-host receipts and orphaned host lock/temporary files after rechecking their retention and liveness. Stale `needs_user`, indeterminate, claimed, and nonterminal portfolio work is reported but never auto-cancelled. Every candidate is rechecked; a concurrent change skips that record without aborting the rest of the batch. Archiving preserves JSON/JSONL history and never deletes worktrees, branches, claims, or GitHub history.
