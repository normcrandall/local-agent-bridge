...
The adapter uses Docker's documented Ollama-compatible loopback API at `http://127.0.0.1:12434`, specifically the currently documented `/api/tags` and `/api/chat` surfaces. This is a compatibility dependency, not a bridge-owned or version-negotiated Docker API contract, so the conditional live-contract test probes both response shapes whenever Docker Model Runner is healthy. `DOCKER_MODEL_RUNNER_MODEL` and `DOCKER_MODEL_RUNNER_HOST` are explicit machine overrides. Non-loopback endpoints are rejected because Docker Model Runner's local API is unauthenticated. Use `agents: ["docker"]` for local review. A mixed `docker`/`ollama` roster is safe because Ollama preflight reports unavailable while the selected Docker reviewer route is healthy. The MCP exposes `ask_docker`, durable `continue_docker`, and `get_docker_status`; state is stored owner-only under `~/.local/state/local-agent-bridge/docker-sessions`.
