...
- Mission Control is a live terminal dashboard over the same persisted control plane. Its default operator view is one fixed, full-height grid: repositories on the left, deduplicated work in the center, and concise selected-lane context on the right. Every row is measured by terminal display width, so ANSI color and Unicode cannot move a divider; narrow terminals collapse to one pane rather than damaging the grid. Semantic color distinguishes active work, stopped-provider user boundaries, waiting work, failures, selection, and stale narrative, while labels and symbols preserve the same meaning under `NO_COLOR` or `--no-color`. Repeated issue/PR identities become one operator row with a related-lane count, repetitive provider-tool messages are suppressed, and consecutive identical timeline events are coalesced. Full UUIDs, workspace paths, timing, and provider activity stay behind detail expansion instead of dominating the default screen.
