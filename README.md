...
`needs_user` is also a machine-level attention signal, not only a stored status. The broker immediately sends a desktop notification with sound, and the persistent supervisor repeats it every 15 minutes until the wake is acknowledged. Delivery is deduplicated and recorded on the wake; failed adapters retry with bounded exponential backoff. Notification previews are generic and keep repository names and protected decision details inside the durable receipt; set `AGENT_BRIDGE_ATTENTION_DETAIL=repository` only when repository names are safe to expose in desktop previews. Mission Control always shows a red `USER INPUT REQUIRED` banner—even in live mode—and rings the terminal bell once for each newly observed wake. Use `bridge attention list` to inspect pending requests, `bridge attention notify` to resend them, and `bridge attention test` to verify the desktop notification path. Set `AGENT_BRIDGE_ATTENTION_NOTIFICATIONS=off` only when desktop alerts must be disabled; durable state and Mission Control warnings remain active without periodic notification writes.
