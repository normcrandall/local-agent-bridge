...
The JSON contains the raw samples, p95 latency, memory, scale exponents, budgets, failures, and the resulting Rust classification. The script writes that JSON before setting a failing exit code, so CI retains the evidence even when a hard budget is exceeded. `MISSION_CONTROL_SCALE_BUDGETS` in `src/mission-control-benchmark.mjs` is the machine-readable source of truth.
...
The implementation run on 2026-07-26 used Node v24.14.0 on Darwin arm64 and passed every hard budget:

| Scenario | Measured p95 |
| --- | ---: |
...
The history-sensitivity probes are more important than the absolute latency:
...
An incremental event cost should be largely independent of unrelated history size; the machine-readable diagnostic threshold is exponent 0.4. Both operations exceed it. The reducer currently creates replacement collection objects for updates and scans collections for removals. That is algorithmic scaling, not evidence that JavaScript instruction dispatch is the bottleneck.

## Decision

Do not move Mission Control to Rust yet.

First keep the existing JavaScript protocol and replace whole-collection copy/scan work with indexed incremental state:
...
4. rerun this exact harness and compare both absolute p95 latency and the history-sensitivity exponents.
