...
For governed delivery, bind `push_branch` and `ensure_pull_request` at implementation start so hydration can prove the future route, while `workProfile: implement` keeps publication disabled and requires no verified head. Implement and commit, record observed verification receipts for the exact clean head, then continue with `workProfile: deliver` and `githubBuilder.verifiedHeadSha` at that head.
