...
For work-mode phases, use the machine-default `github-governed` profile when the goal is bound to a GitHub issue and PR. Pass the concise objective, `issueTarget` (or `issueClaim`), and self-contained checkout; the broker hydrates the issue and derives the initial minimal writer-App binding. Never duplicate issue contents in prose or omit the target to bypass policy. Pass `deliveryProfile: "local-only"` only for deliberately unpublished work and omit all GitHub bindings.

For governed delivery, use `workProfile: implement`; the broker derives or validates `push_branch` and `ensure_pull_request` so hydration can prove the future route, retains and withholds it from the provider, and requires no verified head. Implement and commit, record observed verification receipts for the exact clean head, then continue with `workProfile: deliver` and `githubBuilder.verifiedHeadSha` at that head.
...
Require an exact-head `APPROVED` review from a configured reviewer App, the optional App-authored exact-head `agent-review=success` gate, or an exact-head `APPROVED` review from a machine-locally configured trusted human before autonomous merge. A bot review is evidence but does not satisfy GitHub's nonzero human-approval count. If repository rules still require a human, pause for that person's real review; never retry the merge or review through a personal PAT. PAT compatibility is comment-only for review and cannot publish either gate.

Start each autonomous review phase with an ordered roster of the preferred reviewer and every eligible non-writer fallback in one collaboration. Use `maxTurns` for the number of successful reviews required; failed providers do not consume a turn. A reviewer-App preflight failure makes that participant local-only rather than killing its model review. Prefer any publishable reviewer first; if none can publish, retain the local handoff and require an exact-head approval from a configured trusted human. A critical review may use one candidate only when the user explicitly pins that provider.
