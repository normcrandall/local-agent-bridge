...
Require the App-authored exact-head `agent-review=success` gate before autonomous merge. A bot review is evidence but does not satisfy GitHub's nonzero human-approval count. If repository rules still require a human, stop for that human; never retry the merge or review through a personal PAT. PAT compatibility is comment-only for review and cannot publish the gate.
