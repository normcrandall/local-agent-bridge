...
Use the machine-default `github-governed` delivery profile for issue/PR implementation. Pass the concise objective, `issueTarget` (or `issueClaim`), and self-contained checkout; the broker hydrates the issue and derives the minimal writer-App binding. Never paste issue contents into task prose or omit the target to bypass policy. Pass `deliveryProfile: "local-only"` only for deliberately unpublished work and omit all GitHub targets, claims, and builders.

Governed implementation starts under `workProfile: implement`; the broker derives or validates `push_branch` and `ensure_pull_request` for hydration, retains and withholds those operations from the provider, and requires no verified head during implementation. Governed publication follows a verified continuation: record observed verification receipts for the committed clean head, then switch to `workProfile: deliver` with `githubBuilder.verifiedHeadSha` set to that head.
