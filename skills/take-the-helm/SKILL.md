...
A `Decide:`-class owner-gated fork and the 12-round item circuit-breaker interrupt any mode; the 5-round follow-up breaker applies within the active mode. No run mode releases an owner-gated `Decide:` fork or hard escalation boundary — those always require the existing release authority. Dependency edges order work within a `phase/stop-boundary` value; the value is a coarse partition and checkpoint, not a concurrency fence. When the current phase cannot fill safe capacity, select dependency-ready, deconflicted work from later phases and mark it as lookahead. Consumers map the attribute to whatever field they use — for example a GitHub Project `Phase` field.
...
Default to `maxParallel: 5`, matching the machine's default writer capacity. Continuously fill every available slot from the full dependency-ready backlog, including later phases, only when independent issues, healthy writers, isolated worktrees, claims, footprints, and repository resources are safe. Reduce it automatically when a provider is unavailable or the safe frontier is smaller. Sequential execution is the correct degraded mode when only one safe lane exists.
...
Call `plan_portfolio` before mutation. Reject dependency cycles. Do not treat an open PR, passing branch CI, or a provider handoff as satisfying a hard dependency that requires merged behavior. Recompute the frontier whenever an issue, PR, target branch, provider, or lane manifest changes, and immediately backfill freed capacity. Look across phase boundaries before accepting an idle slot; phase lookahead never overrides a dependency, conflict, claim, footprint, resource, checkout, review, CI, or merge gate.
...
3. **Implement with one writer** — select `workProfile: implement` for local delivery through commit or `workProfile: deliver` for authorized push and PR creation. No other lane or reviewer may write that worktree.
   GitHub issue lanes use the machine-default `github-governed` delivery profile with their bound issue, self-contained checkout, and builder App. Only a deliberately unpublished lane passes `deliveryProfile: "local-only"`, and it must carry no GitHub target, claim, or builder.
   Governed publication is a second phase: record observed verification receipts for the committed clean head, then continue with `githubBuilder.verifiedHeadSha` set to that head and enable `ensure_pull_request`. Never publish a mutable or receipt-free writer head.
