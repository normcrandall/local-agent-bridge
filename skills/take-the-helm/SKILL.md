...
1. **Claim and reserve** — atomically acquire the GitHub-side issue claim lease before worktree creation or delegation, create its worktree and branch, record its expected paths/resources, writer, and collaboration ID with `update_portfolio_item`, then recheck that it does not overlap another active lane. Release claim leases only at terminal delivery (merge, cancel, obsolete) or explicit recovery.
