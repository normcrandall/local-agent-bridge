...
6. **Review independently** — assign providers that did not write the lane. When the PR is the source of truth, use the exact PR head and configured reviewer Apps, and require the resulting exact-head `agent-review` gate. A PAT compatibility comment is not approval. Do not let provider capacity exceed one live call per provider.
