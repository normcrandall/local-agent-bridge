...
- For governed implementation, bind `githubBuilder.allowedOperations` with `push_branch` and `ensure_pull_request` at start so hydration can prove the future publication route, but use `workProfile: implement`; the route remains unusable and no verified head is required in that phase. Publish only after the chair records exact-head observed verification receipts, then continue with `workProfile: deliver` and `githubBuilder.verifiedHeadSha` set to that clean head.
