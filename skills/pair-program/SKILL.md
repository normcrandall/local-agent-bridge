...
- Use the machine-default `github-governed` profile for issue/PR work. Pass only the concise objective, `issueTarget` (or `issueClaim`), and self-contained checkout at implementation start; the broker hydrates the issue and derives the minimal writer-App binding. Never paste the issue body into task prose or omit the target to evade delivery policy. Pass `deliveryProfile: "local-only"` only for intentionally unpublished work and omit every GitHub target, claim, and builder.
- For governed implementation, use `workProfile: implement`; the broker derives or validates `push_branch` and `ensure_pull_request`, retains and withholds the route from the provider, and requires no verified head in that phase. Publish only after the chair records exact-head observed verification receipts, then continue with `workProfile: deliver` and `githubBuilder.verifiedHeadSha` set to that clean head.
